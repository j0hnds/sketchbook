...
/**
   ...
   This will return 0 if no ping sensor is connected or the distance is 
...
int mvToDistance(int mV) {
  ...
  if (mV > INTERVAL * TABLE_ENTRIES) {
    ...
    result = distance[TABLE_ENTRIES-1];
    ...
    int resultInCm = map(mV, mV0, mV1, distance[index-1], distance[index]);  
    result = map(resultInCm, 0, 200, 0, 79); // convert from cm to inches 
